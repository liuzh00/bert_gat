...
def parse_args():
    ...
    parser.add_argument('--data_dir', default='dataset/T', help='TACRED directory.')
    parser.add_argument('--vocab_dir', default='dataset/Laptops', help='Output vocab directory.')
...
def main():
    ...
    # dependent position
    vocab_deppost_file = args.vocab_dir + '/vocab_deppost.vocab'
    # pos_tag
    ...
    dep_counter = Counter(train_dep+test_dep+['self'])
    max_len = max(train_max_len, test_max_len)
    post_counter = Counter(list(range(-max_len, max_len)))
    deppost_counter = Counter(list(range(0, 8)))
    ...
    deppost_vocab = VocabHelp(deppost_counter, specials=['<pad>', '<unk>'])
    ...
    deppost_vocab.save_vocab(vocab_deppost_file)
